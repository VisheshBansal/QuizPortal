...
const Quiz = require(join(__dirname, '..', 'models', 'quiz.model'))
...
exports.deleteQuiz = async (req, res) => {
  ...
  try {
    const quiz = await Quiz.findById(id)
    if (!quiz) { return res.status(404).json({ success: false, error: 'Quiz does not exist' }) }
    console.log(quiz.creator.toString())
...
exports.getQuizzes = async (req, res) => {
  ...
  try {
    const groupQuizzes = await Quiz.find();
    if (!groupQuizzes) { return res.status(404).json({ success: false, error: 'No quizzes' }) }
